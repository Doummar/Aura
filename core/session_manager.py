...
class SessionManager:
    ...
    def reset_phase(self):
        """Restart a fresh study phase, discarding the remaining timer.
        Session statistics (cards, score, streaks) are preserved."""
        if not self.is_active:
            return
        self.pending_break_offer = False
        self.pending_return_offer = False
    ...
    def card_answered(self, latency, ease):
        ...
        is_again = ease == 1
        # Store the actual ease value (1-4) so the configurable per-button
        # weights can be applied in calculate_cognitive_score().
        self.latencies.append((latency, int(ease)))
    ...
    def calculate_cognitive_score(self):
        ...
        min_cards = max(3, min(20, int(self.config.get("calibration_cards", 5))))
        if len(self.latencies) < 2:
            return int(round(self.smoothed_score))

        # ── User-configured weights ───────────────────────────────────────
        weight_map = {
            1: max(-2.0, min(2.0, float(self.config.get("weight_again", -2)))),
            2: max(-2.0, min(2.0, float(self.config.get("weight_hard",  -1)))),
            3: max(-2.0, min(2.0, float(self.config.get("weight_good",   1)))),
            4: max(-2.0, min(2.0, float(self.config.get("weight_easy",   0)))),
        }
        rt_weight   = max(0.0, min(2.0, float(self.config.get("response_time_weight", 1))))
        ignore_easy = bool(self.config.get("ignore_easy_time", False))
        sensitivity = max(0.0, min(1.0, float(self.config.get("sensitivity", 0.5))))
        baseline    = 50.0

        recent = self.latencies[-30:]

        # ── Answer-weight signal ──────────────────────────────────────────
        # Average weight per card mapped linearly:
        #   avg -2 → -50 pts, avg 0 → baseline, avg +2 → +50 pts
        weighted_sum = sum(weight_map.get(ease, 0.0) for (_, ease) in recent)
        avg_weight   = weighted_sum / len(recent)
        acc_delta    = (avg_weight / 2.0) * 50.0

        # ── Response-time signal ──────────────────────────────────────────
        jitter_delta = 0.0
        if rt_weight > 0:
            entries = (
                [(lat, e) for (lat, e) in recent if e != 4]
                if ignore_easy else list(recent)
            )
            if len(entries) >= 2:
                lats   = [x[0] for x in entries]
                avg    = sum(lats) / len(lats)
                var    = sum((x - avg) ** 2 for x in lats) / (len(lats) + 1e-9)
                jitter = math.sqrt(var) / (avg + 0.1)
                # rt_weight=1 → same scale as original; rt_weight=2 → doubled
                jitter_delta = (0.35 - jitter) * 34.0 * rt_weight
    ...
    def get_session_summary(self):
        ...
        if len(self.score_samples) > 0:
            avg_score = sum(self.score_samples) / float(len(self.score_samples))
